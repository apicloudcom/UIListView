...
public class UIListView extends UZModule {
	...
	private String headerLoadingText = "正在加载...";
	private String headerLastUpdateText = "上次更新时间:";
	
	private String footerLoadingText = "正在加载...";
	private String footerLastUpdateText = "上次更新时间:";
	...
	public void jsmethod_open(final UZModuleContext moduleContext) {
		...
		mAdapter = new CustomListAdapter(contex, moduleContext, mConfig.itemDatas, mConfig, getWidgetInfo());
		...
		if(mConfig.showScrollBar){
			swipeListView.setVerticalScrollBarEnabled(true);
		} else {
	...
	private class OnRefreshListener implements OnRefreshListener2<ListView> {
		...
		public void onPullDownToRefresh(PullToRefreshBase<ListView> refreshView) {
			if (showHeaderTime) {
				...
				refreshView.getLoadingLayoutProxy().setLastUpdatedLabel(headerLastUpdateText + label);
		...
		public void onPullUpToRefresh(PullToRefreshBase<ListView> refreshView) {
			if (showFooterTime) {
				SimpleDateFormat dataFormat = new SimpleDateFormat("yyyy/MM/dd hh:mm");
				String label = dataFormat.format(new Date());
				refreshView.getLoadingLayoutProxy().setLastUpdatedLabel(footerLastUpdateText + label);
			}
	...
	public void jsmethod_setRefreshHeader(UZModuleContext uzContext) {
		...
		if (mRefreshableList != null) {
			...
			if(!uzContext.isNull("loadingText")){
				headerLoadingText = uzContext.optString("loadingText");
			}
			
			if(!uzContext.isNull("lastUpdateText")){
				headerLastUpdateText = uzContext.optString("lastUpdateText");
			}
			...
			loadingLayout.setRefreshingLabel(headerLoadingText);
	...
	public void jsmethod_setRefreshFooter(UZModuleContext uzContext) {
		...
		if (mRefreshableList != null) {
			...
			if(!uzContext.isNull("loadingText")){
				footerLoadingText = uzContext.optString("loadingText");
			}
			
			if(!uzContext.isNull("lastUpdateText")){
				footerLastUpdateText = uzContext.optString("lastUpdateText");
			}
			...
			loadingLayout.setRefreshingLabel(footerLoadingText);
