...
public class Config {
	...
	public Config(UZModuleContext uzContext, Context context) {
		...
		if(!uzContext.isNull("showScrollBar")){
			showScrollBar = uzContext.optBoolean("showScrollBar", true);
		}
		...
		if (stylesObj != null) {
			...
			if (itemStyleObj != null) {
				...
				if(!itemStyleObj.isNull("imgCorner")){
					itemImgCorner = UZUtility.dipToPix(itemStyleObj.optInt("imgCorner"));
				}
			...
			JSONObject forbiddenObj = stylesObj.optJSONObject("forbidden");
			if(forbiddenObj != null){
				forbidenBgColor = forbiddenObj.optString("bgColor");
				forbidenRemarkColor = forbiddenObj.optString("remarkColor");
				forbidenSubTitleColor = forbiddenObj.optString("subTitleColor");
				forbidenTitleColor = forbiddenObj.optString("titleColor");
				forbidenSideslip = forbiddenObj.optBoolean("sideslip", false);
			}
		}
